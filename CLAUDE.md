...
Web structure: four routes resolved once in `main.tsx` (`/`, `/create`, `/manage/:id`, `/i/:id` — no router library, see the roadmap backlog for when that changes). The screens are composition only — state lives in `web/src/hooks/` (`useInvitationEditor` = what the invitation is, `usePublishing` = where it's published, `useHostManage` = the host's access to responses, `usePublishedInvitation` + `useRsvpForm` on the guest side) and markup in `web/src/components/{editor,guest,manage}/`. Put new state in a hook rather than in a screen: the hooks are what the tests drive (vitest runs under jsdom, `@testing-library/react` for hooks/components), and `App.tsx`/`GuestPage.tsx`/`ManagePage.tsx` are meant to stay readable at a glance. **`vite.config.ts` sets `globals: false`, so RTL never auto-cleans** — component tests using `screen` must `afterEach(cleanup)` or they start matching the previous test's DOM.
