...
Web structure: the two screens are composition only — state lives in `web/src/hooks/` (`useInvitationEditor` = what the invitation is, `usePublishing` = where it's published, `usePublishedInvitation` + `useRsvpForm` on the guest side) and markup in `web/src/components/editor/` and `web/src/components/guest/`. Put new editor state in a hook rather than in `App.tsx`: the hooks are what the tests drive (vitest runs under jsdom, `@testing-library/react` for hooks/components), and `App.tsx`/`GuestPage.tsx` are meant to stay readable at a glance.
