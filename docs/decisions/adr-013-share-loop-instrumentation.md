...
to the product — the `gr-brand` HOSTYMO wordmark on
...
its own comment, carried over from the DS `guest-rsvp` template: **"HOSTYMO
...
- **The call to action hides the wordmark from its accessible name.** The
  `HOSTYMO` span is `aria-hidden`, so the link reads as "create your own
  invitation" rather than "HOSTYMO create your own invitation" — the wordmark is
