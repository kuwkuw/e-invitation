...
screen need), a web manifest, `theme-color`, and a 1200×630 share card —
`/og-cover.png` and `/og-cover-en.png`, one per language. The cards and the
icons are **committed build artifacts**, generated by
`scripts/build-brand-assets.mjs` from the favicon and a layout in that script:
none of them varies per request, so a crawler should get a static file rather
than a satori render, and generating them keeps the icons from drifting from
the favicon.

The card varies per *language*, which is a different thing and a countable one
— two files, not a renderer. The card carries the headline and the occasion
words, so a single Ukrainian card would mean the English page of §5, the one
addressable page written for English readers, unfurls in a script its reader
followed a link to avoid. Its copy mirrors `LANDING` by hand for the same
reason `SEO_STRINGS` does: a raster cannot import a table.
